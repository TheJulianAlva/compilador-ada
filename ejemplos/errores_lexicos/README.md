...
El analizador léxico reconoce cuatro clases de error, todas con
recuperación (nunca aborta):

| Token trampa / regla | Qué detecta |
|---|---|
| `<ERROR_LEXICO: ~[]>` | cualquier carácter que ningún token acepta (`$ ? @ # \ ! ~` …) |
...
| `<IDENT_MALFORMADO>` | identificador que termina en `_` o contiene `__` |
...
|---|---|
| `01_caracter_no_valido.ada` | 3 — `'$'` (8:11), `'?'` (9:11), `'@'` (10:11) |
...
| `06_varios_lexicos.ada` | 10 — una cadena sin cerrar, un `'ab'`, `dato__x` y `n_` en cada aparición, `'#'` (12:18) y `'\'` (13:23) sueltos |
